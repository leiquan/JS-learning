...
var mkdir = function mkdir(dir) {
  ...
  think.mkdir(dir);
  log(function (colors) {
    return colors.cyan('create') + ' : ' + _path2['default'].normalize(dir);
  });
};
...
var getProjectAppPath = function getProjectAppPath() {
  ...
  path += _commander2['default'].es6 ? 'src' : 'app';
  return path;
};
/**
 * get app name
...
var copyFile = function copyFile(source, target, replace, showWarning) {
  ...
  if (_commander2['default'].ts) {
    var tsSource = source.replace(/([^/]+)$/, function (a, b) {
      return '/ts_' + b;
    });
  ...
  //ECMAScript 6/7
  else if (_commander2['default'].es6) {
  ...
  var content = _fs2['default'].readFileSync(templatePath + '/' + source, 'utf8');
  //replace content
  if (replace) {
  ...
  log(function (colors) {
    return colors.cyan('create') + ' : ' + _path2['default'].normalize(target);
  });
};
...
var parseAppConfig = function parseAppConfig() {
  ...
  _commander2['default'].es6 = data.es6;
...
_commander2['default'].option('-e, --es6', 'use es6 for project, used in `new` command');
...
_commander2['default'].option('-m, --mode <mode>', 'project mode type(mini, normal, module), default is module, used in `new` command', function (mode) {
...
_commander2['default'].command('new <projectPath>').description('create project').action(function (projectPath) {
  projectRootPath = _path2['default'].normalize(projectPath + '/');
  createProject();
});
...
_commander2['default'].command('plugin <pluginPath>').description('create ThinkJS plugin').action(function (pluginPath) {
  projectRootPath = _path2['default'].normalize(pluginPath + '/');
