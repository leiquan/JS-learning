var Rally = function(){
	...
	this.__construct__ = function(){
		...
		this.topLink = document.getElementById("top");
		this.topLink.onclick = this.onTopClick;
		this.topLink.onmouseover = this.onTopOver;
		this.topLink.onmouseout = this.onTopOut;

		// Back arrow
		this.back = new Arrow(document.getElementById("back"));
		this.back.link.onclick = this.arrowBack;

		// Next arrow
		this.next = new Arrow(document.getElementById("next"));
		this.next.link.onclick = this.arrowNext;

		// Stats section
		this.stats = document.getElementById("stats");
		this.statsLight = document.getElementById("statsLight");
		this.statsDark = document.getElementById("statsDark");
		...
		for(var i=0; i<divs.length; i++){
			...
			// Find pages
			if(/page/.test(div.className)){
				var page = new RallyPage(div);
				...
				if(div.id == id){
					...
					// Show first div
					page.div.style.marginLeft = "0";
					page.content.style.opacity = "1";
					page.intro.style.opacity = "1";
				}
			}
		}
	...
	this.onResize = function(e){
		...
		var h = self.currentPage.div.offsetHeight;
		self.pagesDiv.style.height = h + "px";

		self.arrows.style.left = Math.round(s.width / 2) + "px";
	}
...
var RallyNav = function(div){
	var self = this;
	this.div = div;
	this.underline;
	this.selected;
	this.lineProps = {x: 0, width: 0};
	...
	this.__construct__ = function(){
		...
		for(var i=0; i<this.items.length; i++){
			...
			// Set item xpos
			item.link.xpos = xpos;
			xpos += (item.link.offsetWidth + 28);

			// Set item color
			item.link.color = new RGBColor(172, 171, 171);
			item.link.style.color = item.link.color.getHEX().toStringCSS();

			// Get selected item
			if(/selected/.test(item.className)){
				this.selected = item;
				item.link.color = new RGBColor(250, 245, 241);
				item.link.style.color = item.link.color.getHEX().toStringCSS();
			}

			// Dim link
			item.link.brighten = function(e){
				var my = this;
				if(this.colorTween) this.colorTween.stop();
				this.colorTween = new Tween(this.color, 400, {r: 250, g: 245, b: 241, ease: Ease.easeOut.sine, onUpdate: function(){
					my.style.color = my.color.getHEX().toStringCSS();
				}});
			}

			// Brighten link
			item.link.dim = function(e){
				var my = this;
				if(this.colorTween) this.colorTween.stop();
				this.colorTween = new Tween(this.color, 400, {r: 172, g: 171, b: 171, ease: Ease.easeOut.sine, onUpdate: function(){
					my.style.color = my.color.getHEX().toStringCSS();
				}});
			}
			...
			item.onmouseover = function(e){
				if(this != self.selected) self.selected.link.dim();
				this.link.brighten();
			}
			...
			item.onclick = function(e){
				if(this != self.selected){
					GlobalEvents.dispatch({name: "NavClick", item: this});
					self.moveLine(this.link);
					self.selected = this;
				}
		...
		this.list.onmouseout = function(){
			self.selected.link.brighten();
		}

		setTimeout(function(){
			self.moveLine(self.selected.link);
		}, 250);
	}

	// Position the line
	this.positionLine = function(){
		var style = self.underline.style;
		style.left = self.lineProps.x + "px";
		style.width = self.lineProps.width + "px";
	}

	// Animate line under link
	this.moveLine = function(link){
		if(this.lineTween) this.lineTween.stop();
		...
		var props = {
			...
			ease: Ease.easeOut.quint,
			onUpdate: this.positionLine
		}

		// Animate
		this.lineTween = new Tween(this.lineProps, 700, props);
	}
	...
	this.select = function(index){
		...
		// Dim old item
		this.selected.link.dim();
		this.selected = item;

		// Brighten new item
		this.moveLine(item.link);
		item.link.brighten();
	}
...
var Arrow = function(link){
	var self = this;
	this.link = link;
	this.disabled = false;

	this.__construct__ = function(){
		this.sprite = new TraditionalSprite(NavArrow, this.link);
	...
	this.mouseOver = function(e){
		...
		GlobalEvents.dispatch({name: "ArrowOver", arrow: self});
		self.sprite.stop();
	...
	this.mouseOut = function(e){
		if(self.disabled) return false;
		GlobalEvents.dispatch({name: "ArrowOut", arrow: self});
		self.sprite.stop();
